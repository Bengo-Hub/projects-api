...
APP_NAME=${APP_NAME:-"projects-service"}
NAMESPACE=${NAMESPACE:-"projects"}
ENV_SECRET_NAME=${ENV_SECRET_NAME:-"projects-service-secrets"}
DEPLOY=${DEPLOY:-true}
...
if [[ ${DEPLOY} == "true" ]]; then
  ...
  if curl -fsSL https://raw.githubusercontent.com/Bengo-Hub/devops-k8s/main/scripts/tools/check-and-sync-secrets.sh -o "$SYNC_SCRIPT" 2>/dev/null; then
    source "$SYNC_SCRIPT"
    check_and_sync_secrets "REGISTRY_USERNAME" "REGISTRY_PASSWORD" "GIT_TOKEN" "POSTGRES_PASSWORD" "REDIS_PASSWORD" || warn "Secret sync failed - continuing with existing secrets"
    rm -f "$SYNC_SCRIPT"
  else
...
if ! kubectl -n "$NAMESPACE" get secret "$ENV_SECRET_NAME" >/dev/null 2>&1; then
  ...
  kubectl -n "$NAMESPACE" create secret generic "$ENV_SECRET_NAME" \
    --from-literal=PROJECTS_POSTGRES_URL="postgresql://${SERVICE_DB_USER}:PASSWORD@postgresql.infra.svc.cluster.local:5432/${SERVICE_DB_NAME}?sslmode=disable" \
    --from-literal=PROJECTS_REDIS_ADDR="redis-master.infra.svc.cluster.local:6379" \
    --from-literal=PROJECTS_NATS_URL="nats://nats.messaging.svc.cluster.local:4222" || true
fi
