...
func (_q *AttachmentQuery) QueryProject() *ProjectQuery {
...
func (_q *AttachmentQuery) QueryTask() *TaskQuery {
...
func (_q *AttachmentQuery) WithProject(opts ...func(*ProjectQuery)) *AttachmentQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
func (_q *AttachmentQuery) WithTask(opts ...func(*TaskQuery)) *AttachmentQuery {
	query := (&TaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AttachmentQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Project)) error {
...
func (_q *AttachmentQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Task)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attachment)
	for i := range nodes {
		fk := nodes[i].TaskID
