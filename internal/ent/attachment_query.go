...
func (aq *AttachmentQuery) QueryProject() *ProjectQuery {
...
func (aq *AttachmentQuery) QueryTask() *TaskQuery {
...
func (aq *AttachmentQuery) WithProject(opts ...func(*ProjectQuery)) *AttachmentQuery {
	query := (&ProjectClient{config: aq.config}).Query()
...
func (aq *AttachmentQuery) WithTask(opts ...func(*TaskQuery)) *AttachmentQuery {
	query := (&TaskClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(attachment.FieldTenantID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Attachment.Query().
//		Select(attachment.FieldTenantID).
//		Scan(ctx, &v)
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	...
	if query := aq.withProject; query != nil {
		if err := aq.loadProject(ctx, query, nodes, nil,
			func(n *Attachment, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTask; query != nil {
		if err := aq.loadTask(ctx, query, nodes, nil,
			func(n *Attachment, e *Task) { n.Edges.Task = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttachmentQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Project)) error {
...
func (aq *AttachmentQuery) loadTask(ctx context.Context, query *TaskQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Task)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attachment)
	for i := range nodes {
		fk := nodes[i].TaskID
...
func (aq *AttachmentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withTask != nil {
			_spec.Node.AddColumnOnce(attachment.FieldTaskID)
		}
	}
