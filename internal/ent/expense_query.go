...
func (eq *ExpenseQuery) QueryBudget() *BudgetQuery {
	query := (&BudgetClient{config: eq.config}).Query()
...
func (eq *ExpenseQuery) Clone() *ExpenseQuery {
	...
	return &ExpenseQuery{
		...
		withBudget: eq.withBudget.Clone(),
...
func (eq *ExpenseQuery) WithBudget(opts ...func(*BudgetQuery)) *ExpenseQuery {
	query := (&BudgetClient{config: eq.config}).Query()
...
func (eq *ExpenseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Expense, error) {
	...
	if query := eq.withBudget; query != nil {
		if err := eq.loadBudget(ctx, query, nodes, nil,
			func(n *Expense, e *Budget) { n.Edges.Budget = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *ExpenseQuery) loadBudget(ctx context.Context, query *BudgetQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *Budget)) error {
...
func (eq *ExpenseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withBudget != nil {
			_spec.Node.AddColumnOnce(expense.FieldBudgetID)
		}
	}
