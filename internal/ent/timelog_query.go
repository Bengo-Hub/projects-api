...
func (tlq *TimeLogQuery) QueryProject() *ProjectQuery {
	query := (&ProjectClient{config: tlq.config}).Query()
...
func (tlq *TimeLogQuery) Clone() *TimeLogQuery {
	...
	return &TimeLogQuery{
		...
		withProject: tlq.withProject.Clone(),
...
func (tlq *TimeLogQuery) WithProject(opts ...func(*ProjectQuery)) *TimeLogQuery {
	query := (&ProjectClient{config: tlq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tlq.withProject = query
	return tlq
}
...
//		GroupBy(timelog.FieldProjectID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (tlq *TimeLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*TimeLog, error) {
	...
	if query := tlq.withProject; query != nil {
		if err := tlq.loadProject(ctx, query, nodes, nil,
			func(n *TimeLog, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tlq *TimeLogQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*TimeLog, init func(*TimeLog), assign func(*TimeLog, *Project)) error {
...
func (tlq *TimeLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tlq.ctx.Fields; len(fields) > 0 {
		...
		if tlq.withProject != nil {
			_spec.Node.AddColumnOnce(timelog.FieldProjectID)
		}
	}
